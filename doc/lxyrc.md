...
You might also want to replace the track motors leads and solder one 0.1uF ceramic capacitor from each terminal to the motor case to protect the internals from electrical noise coming from the motors.
...
In order to be able to change BlHeli settings later when the model was fully assembled, I wrote a separate piece of stub firmware called [active low signal passthrough](passthru.md) that allowed to configure a BlHeli ESC via the SWIM pin.
