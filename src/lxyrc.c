...
void update(void) {
	...
	if (b != bm) { // Change mode atomically
		TIM3_CR1 = 0x00; // Disable counter
		TIM3_CCMR1 = 0x40; // OC1M=100 (force low)
		TIM3_CCMR2 = 0x40; // OC2M=100 (force low)
		if (b & 1) TIM3_CCMR1 = 0x30; // OC1M=011 (toggle)
		if (b & 2) TIM3_CCMR2 = 0x30; // OC2M=011 (toggle)
		if (b & 4) { // Strobe
		...
		TIM3_EGR = 0x01; // UG=1 (reset counter)
